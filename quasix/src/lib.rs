...
use quasix_core::selfenergy::{
    CorrelationSelfEnergyCD, ExchangeSelfEnergyRI,
};
...
fn compute_cholesky_metric<'py>(
    ...
    let cholesky = compute_cholesky_v_with_tolerance(&metric_array, None, Some(1e-2)).map_err(|e| {
        PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
fn solve_quasiparticle_equations_DISABLED<'py>(
    ...
    let sigma_total_func =
        move |orbital_idx: usize, energy: f64| -> quasix_core::common::Result<Complex64> {
            ...
            let sigma_c_dynamic = match energy_model.as_str() {
                ...
                "linear" => {
                    ...
                    let linear_factor =
                        (1.0 - energy_dependence_alpha * energy_diff).clamp(0.1, 2.0);
...
fn compute_exchange_symmetric<'py>(
    ...
    let df_3c_mo_sym_array = df_3c_mo_sym.as_array().to_owned();
...
fn compute_correlation_self_energy_cd_DISABLED<'py>(
    ...
    dict.set_item(
        "sigma_c_real",
        PyArray2::from_owned_array(py, sigma_c_real),
    )?;
    dict.set_item(
        "sigma_c_imag",
        PyArray2::from_owned_array(py, sigma_c_imag),
    )?;
    ...
    dict.set_item(
        "residue_real",
        PyArray2::from_owned_array(py, residue_real),
    )?;
    dict.set_item(
        "residue_imag",
        PyArray2::from_owned_array(py, residue_imag),
    )?;
...
fn compute_screened_interaction_s33_DISABLED<'py>(
    ...
    let condition_number = solver.estimate_condition(&one_minus_m).unwrap_or(f64::INFINITY);
...
fn compute_dielectric_function<'py>(
    ...
    dict.set_item(
        "epsilon",
        PyArray2::from_owned_array(py, epsilon_real),
    )?;
...
fn compute_screened_interaction_batch_DISABLED<'py>(
    ...
    // Create solver
    let solver = DielectricSolver::with_config(naux, solver_type, SolverBackend::Auto, solver_config);
...
fn create_optimized_gl_grid<'py>(
    ...
    result.set_item(
        "nodes",
        PyArray1::from_owned_array(py, grid.nodes.clone()),
    )?;
    result.set_item(
        "weights",
...
fn create_contour_deformation(
    ...
    Python::with_gil(|py| {
        ...
        config.insert("energy_min".to_string(), energy_min.into_pyobject(py)?.unbind().into());
        config.insert("energy_max".to_string(), energy_max.into_pyobject(py)?.unbind().into());
        config.insert("n_points".to_string(), n_points.into_pyobject(py)?.unbind().into());
        config.insert("method".to_string(), "contour_deformation".into_pyobject(py)?.unbind().into());
        Ok(config)
    })
}
...
fn create_ac_fitter(
    ...
    Python::with_gil(|py| {
        let mut config = HashMap::new();
        config.insert("n_poles".to_string(), n_poles.into_pyobject(py)?.unbind().into());
        config.insert("regularization".to_string(), reg.into_pyobject(py)?.unbind().into());
        config.insert(
            "method".to_string(),
