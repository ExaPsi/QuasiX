...
use numpy::{PyArray1, PyReadonlyArray1, PyReadonlyArray2, PyReadonlyArray3, PyUntypedArrayMethods};
use pyo3::prelude::*;
...
use quasix_core::qp::{
    ContourDeformationConfig, EvGWConfig, EvGWDriver, QPSolverConfig,
};
...
pub fn evgw<'py>(
    ...
    let iaP_3d = ndarray::Array3::from_shape_vec(
        ...
        format!("Failed to reshape iaP tensor: {}", e)
    ))?;
    ...
    let miP_3d = ndarray::Array3::from_shape_vec(
        (n_mo, n_occ, n_aux),
        ijP_arr.as_slice().unwrap().to_vec(),
    ).map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(
        format!("Failed to reshape miP tensor: {}", e)
    ))?;
    ...
    let abP_3d = ndarray::Array3::from_shape_vec(
        ...
        format!("Failed to reshape abP tensor: {}", e)
    ))?;
    ...
    println!("✓ Full DF tensor built from blocks (optimized): shape {:?}, norm {:.2e}",
             mnP_3d.shape(), mnP_norm);
    ...
    let result = driver.run_evgw_blocks(
        ...
        format!("evGW calculation failed: {}", e)
    ))?;
    ...
    output.insert(
        "n_cycles".to_string(),
        PyInt::new(py, result.n_iterations as i64).to_owned().unbind().into(),
    );
    // Get final error from convergence history
    let final_error = result.convergence_history.max_energy_changes
        .last()
        .copied()
        .unwrap_or(0.0);
    output.insert(
    ...
    for (cycle, energies) in result.convergence_history.energies.iter().enumerate() {
        ...
        iter_dict.set_item("z_factors", PyArray1::from_vec(py, result.z_factors.to_vec()))?;
        let energy_change = result.convergence_history.max_energy_changes.get(cycle).copied().unwrap_or(0.0);
        let rms_change = result.convergence_history.rms_energy_changes.get(cycle).copied().unwrap_or(0.0);
        ...
        iter_dict.set_item("converged", cycle + 1 == result.n_iterations && result.converged)?;
    ...
    output.insert(
        "ip".to_string(),
        PyFloat::new(py, -result.qp_energies[n_occ - 1]).to_owned().unbind().into(),
    );

    if n_vir > 0 {
        output.insert(
            "ea".to_string(),
            PyFloat::new(py, -result.qp_energies[n_occ]).to_owned().unbind().into(),
        );
        output.insert(
            "gap".to_string(),
            PyFloat::new(py, result.qp_energies[n_occ] - result.qp_energies[n_occ - 1]).to_owned().unbind().into(),
        );
    } else {
...
pub fn run_evgw(py: Python, config: &Bound<'_, PyDict>) -> PyResult<HashMap<String, Py<PyAny>>> {
    ...
    let iaP_2d_owned = iaP_arr.to_shape((nocc_check * nvir_check, naux_check))
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyValueError, _>(
            format!("Failed to reshape iaP from 3D to 2D: {}", e)
        ))?.to_owned();
    ...
    let mnP_opt: Option<PyReadonlyArray2<f64>> = config
        .get_item("mnP")?
        .map(|x| x.extract::<PyReadonlyArray2<f64>>())
        .transpose()?;
