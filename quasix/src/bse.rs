...
use quasix_core::bse::{
    analyze_exciton, compute_absorption_spectrum, compute_oscillator_strengths,
    compute_amplitude_heatmap, compute_hole_density, compute_electron_density,
    compute_nto_decomposition, compute_participation_ratio, find_dominant_transition,
    verify_svd_roundtrip, BroadeningType, BSEKernelConfig, BSETDAKernel, DavidsonConfig,
...
impl PyBSETDAKernel {
    ...
    fn new(
        ...
        // Create kernel
        let kernel = BSETDAKernel::new(nocc, nvirt, naux, delta_qp, df_ia, df_ij, df_ab, w0, config)
            .map_err(|e| {
    ...
    fn apply_hamiltonian_batch<'py>(
        ...
        let y = self.inner.apply_tda_hamiltonian_batch(&x_batch).map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
fn py_compute_oscillator_strengths<'py>(
    ...
    let dipoles_2d: Vec<Vec<f64>> = (0..n_roots)
        .map(|i| (0..n_comp).map(|j| result.transition_dipoles[[i, j]]).collect())
        .collect();
...
fn py_compute_absorption_spectrum<'py>(
    ...
    // Call Rust implementation
    let result = compute_absorption_spectrum(eigenvalues, oscillator_strengths, &config).map_err(
        |e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
...
fn py_analyze_exciton<'py>(
    ...
    let result = analyze_exciton(eigenvalues, eigenvectors, nocc, nvirt, state_index).map_err(
        |e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
    ...
    dict.set_item("excitation_energy_eV", result.excitation_energy * HARTREE_TO_EV)?;
...
fn py_find_dominant_transition(
    amplitude_squared: PyReadonlyArray2<f64>,
) -> (usize, usize, f64) {
...
fn py_analyze_exciton_simple<'py>(
    ...
    let amp_squared = compute_amplitude_heatmap(flat_arr.view(), nocc, nvirt)
        .map_err(|e| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                "Failed to compute amplitude heatmap: {}",
                e
