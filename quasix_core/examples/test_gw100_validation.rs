...
fn main() {
    ...
    println!("  Found {} outlier(s) at indices: {:?}", outliers.len(), outliers);
    ...
    println!("  Passes 0.1 eV threshold: {}", result.passes_threshold(0.1));
    ...
    match compute_validation_stats(&results) {
        Ok(stats) => {
            ...
            println!("  Passes validation (MAD < 0.2, R² > 0.9): {}",
