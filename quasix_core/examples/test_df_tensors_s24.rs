...
fn main() -> Result<()> {
    ...
    let data_size = (naux * naux + naux * (nao * (nao + 1) / 2)
                     + (nocc * nvir) * naux + (nocc * nocc) * naux) * 8; // f64 = 8 bytes
    ...
    println!("  Data size: {:.2} KB (uncompressed)", data_size as f64 / 1024.0);
    ...
    // Performance summary
    println!("\nPerformance Summary:");
    println!("  Write speed: {:.1} MB/s", (data_size as f64 / 1024.0 / 1024.0) / write_time.as_secs_f64());
    println!("  Read speed: {:.1} MB/s", (data_size as f64 / 1024.0 / 1024.0) / read_time.as_secs_f64());
