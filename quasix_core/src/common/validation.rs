...
pub fn validate_array1(
    ...
    let nonzero_count = arr.iter().filter(|&&x| x.abs() > config.min_nonzero_threshold).count();
    if nonzero_count == 0 {
        return Err(QuasixError::InvalidInput(format!(
...
pub fn validate_array2(
    ...
    if nonzero_fraction < config.min_nonzero_fraction {
        return Err(QuasixError::InvalidInput(format!(
            ...
            name, nonzero_fraction * 100.0, config.min_nonzero_fraction * 100.0
        )));
    }
...
pub fn validate_array3(
    ...
    let nonzero_count = arr.iter().take(sample_size).filter(|&&x| x.abs() > config.min_nonzero_threshold).count();
...
pub fn validate_mo_energies(energies: &ArrayView1<f64>) -> Result<()> {
    ...
    let max_energy = energies.iter().fold(f64::NEG_INFINITY, |max, &x| max.max(x));
