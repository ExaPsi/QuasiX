...
impl NewtonQPSolver {
        ...
        F: Fn(f64) -> Complex64 + ?Sized,
    {
        ...
        if self.config.verbose > 0 {
            ...
            println!("  ε_HF = {:.6} Ha, Σx = {:.6} Ha, Vxc = {:.6} Ha",
        ...
        for iter in 0..self.config.max_iterations {
            ...
            if self.config.verbose > 1 {
                println!("  Iter {:2}: E = {:.8} Ha, Σc = {:.6} Ha, |f| = {:.2e}",
            ...
            if abs_residual < self.config.energy_tolerance {
                ...
                if self.config.verbose > 0 {
                    println!("  Converged in {} iterations: E_QP = {:.6} Ha (Z = {:.4})",
                             iter + 1, energy, z);
                }
        ...
        F: Fn(f64) -> Complex64 + ?Sized,
    {
        ...
        for iter in 0..self.config.max_bisection_iterations {
            ...
            if f_mid.abs() < self.config.energy_tolerance || (e_high - e_low) / 2.0 < self.config.energy_tolerance {
                let omega = e_mid - self.fermi_level;
                let sigma_c = sigma_c_eval(omega);
                let z = self.compute_z_factor(sigma_c_eval, e_mid);

                if self.config.verbose > 0 {
                    println!("  Bisection converged in {} iterations: E_QP = {:.6} Ha",
                             iter + 1, e_mid);
                }
...
pub fn create_sigma_c_evaluator(
    ...
    Box::new(move |omega: f64| {
        pade_thiele(omega, &omega_fit, &pade_coeff)
    })
}
...
pub fn compare_linearized_vs_solved(
    ...
    let diffs: Vec<f64> = linearized.iter()
        .zip(solved.iter())
        .map(|(l, s)| (l - s).abs())
        .collect();
...
mod tests {
    ...
    fn test_bisection_fallback() {
        ...
        let sigma_c_eval = |omega: f64| {
            Complex64::new(-0.1 / (1.0 + omega * omega), 0.0)
        };
