...
use crate::selfenergy::frequency_cache::{FrequencyCache, get_sigma_diag_and_cache, get_sigma_diag_cached};
...
impl EvGWDriver {
    ...
    pub fn run_evgw_blocks(
        ...
        if self.config.print_level > 0 {
            ...
            println!("============================================================");
            println!(" n_mo: {:4}  n_occ: {:4}  n_vir: {:4}  n_aux: {:4}", n_mo, n_occ, n_vir, n_aux);
            println!(" max_iter: {:2}  conv_tol: {:.0e}  damping: {:.2}",
                     self.config.max_iterations, self.config.energy_tolerance, self.config.damping_factor);
            if self.config.use_diis {
                println!(" DIIS: enabled (subspace dim = {})", self.config.diis_space_dim);
            } else {
                println!(" DIIS: disabled (using simple damping)");
            }
            // Production feature: energy window
            println!(" Energy window: {:.1} Ha ({:.0} eV) around Fermi level",
                     self.config.energy_window, self.config.energy_window * 27.2114);
            println!(" Active orbitals: {} occ (MO {}-{}) + {} vir (MO {}-{})",
                     n_active_occ, active_occ_start, n_occ - 1,
                     n_active_vir, n_occ, active_vir_end - 1);
            if self.config.frontier_convergence {
                println!(" Convergence: frontier orbitals only (window = {:.1} Ha)",
        ...
        if self.config.print_level > 0 {
            ...
            println!("  Sigma_x[HOMO] = {:.6} Ha = {:.4} eV", sigma_x_diag[n_occ - 1], sigma_x_diag[n_occ - 1] * 27.2114);
            println!("  Sigma_x[LUMO] = {:.6} Ha = {:.4} eV", sigma_x_diag[n_occ], sigma_x_diag[n_occ] * 27.2114);
        }
        ...
        let lpq = mn_full.clone().permuted_axes([2, 0, 1]).as_standard_layout().to_owned();

        // Verify dimensions
        debug_assert_eq!(lpq.dim(), (n_aux, n_mo, n_mo),
            "lpq shape mismatch: expected ({}, {}, {}), got {:?}",
            n_aux, n_mo, n_mo, lpq.dim());
        ...
        for iter in 0..self.config.max_iterations {
            ...
            if self.config.print_level > 1 && iter > 0 {
                ...
                println!("    HF gap:  {:.4} eV (HOMO={:.4}, LUMO={:.4})",
                         gap_hf, homo_hf * 27.2114, lumo_hf * 27.2114);
                println!("    QP gap:  {:.4} eV (HOMO={:.4}, LUMO={:.4})",
                         gap_qp, homo_qp * 27.2114, lumo_qp * 27.2114);
            ...
            if self.config.use_solved_qp {
                ...
                for (idx, &p) in orbs.iter().enumerate() {
                    ...
                    let initial_guess = if iter == 0 {
                        ...
                        let z = (1.0 / (1.0 - dsigma_dw)).max(self.config.z_min).min(self.config.z_max);
                    ...
                    let sigma_c_eval = |omega: f64| {
                        correlation_ac::pade_thiele(omega, &omega_p, &coeff_p)
                    };
                    ...
                    match solver.solve(&sigma_c_eval, initial_guess) {
                        ...
                        Err(_) => {
                            ...
                            let sigma_c_plus = correlation_ac::pade_thiele(omega_eval + de, &omega_p, &coeff_p);
                            let dsigma_dw = (sigma_c_plus.re - sigma_c.re) / de;
                            let z = (1.0 / (1.0 - dsigma_dw)).max(self.config.z_min).min(self.config.z_max);
                            new_qp_energies[p] = mo_energy[p] + z * (sigma_x_diag[p] + sigma_c.re - vxc_dft[p]);
                            new_z_factors[p] = z;
            ...
            if iter == 0 {
                ...
                for p in 0..n_mo {
                    qp_energies[p] = (1.0 - damping) * qp_energies_old[p] + damping * new_qp_energies[p];
                ...
                match convergence_monitor.accelerate(&new_qp_energies, &new_z_factors) {
                    Ok((diis_energies, diis_z)) => {
                        ...
                        let max_energy_deviation = diis_energies.iter()
                            .zip(new_qp_energies.iter())
                            .map(|(d, n)| (d - n).abs())
                            .fold(0.0, f64::max);

                        let max_z_deviation = diis_z.iter()
                        ...
                        if max_energy_deviation < 0.5 && max_z_deviation < 0.2 {
                            ...
                            if self.config.print_level > 1 {
                                println!("  [DIIS] Extrapolation applied (dE_max={:.4}, dZ_max={:.4})",
            ...
            let (conv_occ_start, conv_vir_end) = self.determine_convergence_window(mo_energy, n_occ, ef);
            let n_conv_orbs = conv_vir_end - conv_occ_start;

            // Compute energy changes for ALL orbitals (for record-keeping)
            let energy_diff: Vec<f64> = qp_energies.iter()
                .zip(qp_energies_old.iter())
                .map(|(new, old)| (new - old).abs())
                .collect();
            let max_change_all = energy_diff.iter().cloned().fold(0.0, f64::max);
            let _rms_change_all = (energy_diff.iter().map(|x| x * x).sum::<f64>() / n_mo as f64).sqrt();
            ...
            let rms_change = (energy_diff_conv.iter().map(|x| x * x).sum::<f64>() / n_conv_orbs as f64).sqrt();
            ...
            if self.config.print_level > 0 {
                ...
                if self.config.frontier_convergence {
                    println!("\nIteration {:2}: max_dE(frontier) = {:.2e} Ha, max_dE(all) = {:.2e} Ha{}",
                             iter + 1, max_change, max_change_all, accel_status);
                    println!("  Convergence window: MO {}-{} ({} orbitals)",
                             conv_occ_start, conv_vir_end - 1, n_conv_orbs);
                } else {
                    println!("\nIteration {:2}: max_dE = {:.2e} Ha, rms_dE = {:.2e} Ha{}",
                             iter + 1, max_change, rms_change, accel_status);
                }
                println!("  E_QP[HOMO] = {:.6} Ha = {:.4} eV  (Z = {:.4})",
                         qp_energies[n_occ - 1], qp_energies[n_occ - 1] * 27.2114, z_factors[n_occ - 1]);
                println!("  E_QP[LUMO] = {:.6} Ha = {:.4} eV  (Z = {:.4})",
                         qp_energies[n_occ], qp_energies[n_occ] * 27.2114, z_factors[n_occ]);
                println!("  Sigma_c[HOMO] = {:.6} Ha, Sigma_c[LUMO] = {:.6} Ha",
                         final_sigma_c[n_occ - 1].re, final_sigma_c[n_occ].re);
            }
        ...
        // Did not converge
        if self.config.print_level > 0 {
            println!("\n[WARN] evGW did not converge in {} iterations", self.config.max_iterations);
        }
...
pub fn g0w0_linearized(
    ...
    let (sigma_iw, omega_iw) = correlation_ac::get_sigma_diag(mo_energy, lpq, &orbs, n_occ, &config)?;
