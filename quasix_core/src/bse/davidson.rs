...
mod tests {
    ...
    fn create_diagonal_apply_h(diagonal: Array1<f64>) -> impl Fn(&Array2<f64>) -> Result<Array2<f64>> {
        move |v: &Array2<f64>| {
    ...
    fn test_apply_preconditioner() {
        ...
        let precond = apply_preconditioner(residual.view(), qp_gaps.view(), ritz_value, level_shift);
    ...
    fn test_orthogonalize_against_subspace() {
        ...
        let orthogonalized =
            orthogonalize_against_subspace(&v, new_vec, 1e-12).expect("Should not be linearly dependent");
