...
pub fn compute_optical_properties(
    ...
    let spectrum = compute_absorption_spectrum(
        eigenvalues,
        optical.oscillator_strengths.view(),
        config,
    )?;
...
mod tests {
    ...
    fn test_oscillator_strengths_non_negative() {
        ...
        let eigenvectors =
            Array2::from_shape_fn((3, 3), |(i, j)| if i == j { 0.8 } else { 0.1 });
        ...
        for &f in result.oscillator_strengths.iter() {
