...
impl BSETDAKernel {
    ...
    pub fn solve_davidson(
        &self,
        config: &super::DavidsonConfig,
    ) -> Result<super::DavidsonResult> {
        // Create closure that applies H to a batch of vectors
        let apply_h = |v: &Array2<f64>| -> Result<Array2<f64>> {
            self.apply_tda_hamiltonian_batch(v)
        };
...
mod tests {
    ...
    fn test_triplet_no_exchange() {
        ...
        let config = BSEKernelConfig::triplet();
        let kernel =
            BSETDAKernel::new(nocc, nvirt, naux, delta_qp, df_ia, df_ij, df_ab, w0, config).unwrap();
