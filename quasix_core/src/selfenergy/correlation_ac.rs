...
fn gauss_legendre_nodes_weights(n: usize) -> (Array1<f64>, Array1<f64>) {
    ...
    let (eigenvalues, eigenvectors) = jacobi.eigh(ndarray_linalg::UPLO::Upper)
            ...
            let epsilon_inv = match epsilon.invc() {
                ...
                Err(chol_err) => {
                    ...
                    match epsilon.inv() {
                        Ok(inv) => inv,
                        Err(lu_err) => {
                            eprintln!("WARNING: Matrix inversion failed at freq {}: Chol={:?}, LU={:?}",
                                     w, chol_err, lu_err);
...
mod tests {
    ...
    fn test_pade_thiele_evaluation() {
        ...
        assert!(result.im.abs() < 0.1, "Imaginary part should be small: {}", result.im);
