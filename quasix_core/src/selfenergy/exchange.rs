...
pub fn compute_exchange_diagonal(
    ...
    if v_aux_inv.dim() != (n_aux, n_aux) {
        return Err(crate::common::QuasixError::InvalidInput(
            format!("DF metric dimension mismatch: expected ({}, {}), got ({}, {})",
                    n_aux, n_aux, v_aux_inv.dim().0, v_aux_inv.dim().1)
        ));
    }

    if n_occ + n_virt > n_mo {
        return Err(crate::common::QuasixError::InvalidInput(
            format!("Inconsistent MO dimensions: n_occ={}, n_virt={}, n_mo={}",
                    n_occ, n_virt, n_mo)
        ));
    }
...
pub fn compute_exchange_matrix(
    ia_p: &Array3<f64>,
    v_aux_inv: &Array2<f64>,
) -> Result<Array2<f64>> {
    use ndarray::s;
    ...
    let tmp = ndarray::stack(Axis(0), &tmp_views)
        .map_err(|e| crate::common::QuasixError::InvalidInput(
            format!("Failed to stack parallel results: {}", e)
        ))?;
    ...
    let tmp_flat = tmp.into_shape((n_occ, n_virt * n_aux))
        .map_err(|e| crate::common::QuasixError::InvalidInput(
            format!("Failed to reshape tmp: {}", e)
        ))?;

    let ia_p_flat = ia_p.to_owned().into_shape((n_occ, n_virt * n_aux))
        .map_err(|e| crate::common::QuasixError::InvalidInput(
            format!("Failed to reshape ia_p: {}", e)
        ))?;
