...
pub fn compute_sigma_c_pyscf_cd(
    ...
    for n in 0..(nmo.min(5)) {
        eprintln!("  Σᶜ[{}] = {:.8e} Ha = {:.6e} eV (Im: {:.3e})",
                 n, sigma_c[n].re, sigma_c[n].re * 27.211386, sigma_c[n].im);
    }

    // Sign check
    eprintln!("\n=== SIGN CHECK ===");
    if nmo >= 2 {
        let homo_sign = if sigma_c[0].re < 0.0 { "NEGATIVE ✓" } else { "POSITIVE ✗" };
        let lumo_sign = if sigma_c[1].re > 0.0 { "POSITIVE ✓" } else { "NEGATIVE ✗" };
