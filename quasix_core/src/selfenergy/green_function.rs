...
pub fn compute_green_function(
    ...
    if !chemical_potential.is_finite() {
        return Err(QuasixError::InvalidInput(
            format!("Chemical potential must be finite, got: {}", chemical_potential)
        ));
    }
...
mod tests {
    ...
    fn test_green_function_multiple_orbitals() {
        ...
        assert!(mag_high < mag_low, "Green's function should decay with frequency");
    }
    ...
    fn test_green_function_pyscf_convention() {
        ...
        assert!(result_occ.is_ok(), "Should accept negative imaginary frequency");

        let result_vir = compute_green_function(&omega_vir, &mo_energy, mu);
        assert!(result_vir.is_ok(), "Should accept positive imaginary frequency");
    }
