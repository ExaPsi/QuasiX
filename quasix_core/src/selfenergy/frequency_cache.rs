...
use super::correlation_ac::{ACConfig, get_scaled_legendre_roots, get_rho_response};
...
mod tests {
    ...
    fn test_cached_vs_noncached_sigma() {
        ...
        let (sigma_noncached, omega_noncached) = correlation_ac::get_sigma_diag(
            &mo_energy,
            &lpq,
            &orbs,
            nocc,
        ...
        let mut cache = FrequencyCache::new();
        let (sigma_cached, omega_cached) = get_sigma_diag_and_cache(
            &mo_energy,
            &lpq,
            &orbs,
            nocc,
        ...
        let (sigma_from_cache, omega_from_cache) = get_sigma_diag_cached(
            &mo_energy,
            &cache,
            &ac_config,
        ...
        assert_eq!(sigma_noncached.dim(), sigma_cached.dim(),
            "Sigma dimensions should match");
        assert_eq!(sigma_noncached.dim(), sigma_from_cache.dim(),
            "Sigma from cache dimensions should match");
        assert_eq!(omega_noncached.dim(), omega_cached.dim(),
        ...
        for idx in 0..sigma_noncached.len() {
            let diff = (sigma_noncached.as_slice().unwrap()[idx]
                       - sigma_cached.as_slice().unwrap()[idx]).norm();
            max_diff_sigma = max_diff_sigma.max(diff);
        }
        assert!(max_diff_sigma < tol,
        ...
        for idx in 0..sigma_noncached.len() {
            let diff = (sigma_noncached.as_slice().unwrap()[idx]
                       - sigma_from_cache.as_slice().unwrap()[idx]).norm();
            max_diff_from_cache = max_diff_from_cache.max(diff);
        }
        assert!(max_diff_from_cache < tol,
        ...
        for idx in 0..omega_noncached.len() {
            let diff = (omega_noncached.as_slice().unwrap()[idx]
                       - omega_cached.as_slice().unwrap()[idx]).norm();
            max_diff_omega = max_diff_omega.max(diff);
        }
        assert!(max_diff_omega < tol,
            "Omega should match: max_diff = {:.2e}, tol = {:.2e}",
            max_diff_omega, tol);

        println!("Frequency caching validation PASSED:");
        println!("  max_diff(sigma_cached vs noncached) = {:.2e}", max_diff_sigma);
        println!("  max_diff(sigma_from_cache vs noncached) = {:.2e}", max_diff_from_cache);
