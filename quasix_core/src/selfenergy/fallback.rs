...
pub enum FallbackReason {
    ...
    PoleInstability { stability_score: f64, threshold: f64 },
    /// Causality violation detected (wrong sign of Im Σ)
    CausalityViolation { violation_score: f64, threshold: f64 },
...
impl fmt::Display for FallbackReason {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FallbackReason::CrossValidationError { cv_error, threshold } => {
                write!(f, "CV error {:.2e} > threshold {:.2e}", cv_error, threshold)
            }
            FallbackReason::PoleInstability { stability_score, threshold } => {
                write!(f, "Pole stability {:.3} < threshold {:.3}", stability_score, threshold)
            }
            FallbackReason::CausalityViolation { violation_score, threshold } => {
                write!(f, "Causality violation {:.3} > threshold {:.3}", violation_score, threshold)
            }
            FallbackReason::SumRuleViolation { error, threshold } => {
                write!(f, "Sum rule error {:.2e} > threshold {:.2e}", error, threshold)
            }
...
impl FallbackController {
    ...
    pub fn evaluate(&mut self, metrics: &QualityMetrics) -> FallbackDecision {
        ...
        if metrics.band_gap_ev < thresholds.gap_min_ev {
            self.log(&format!("FALLBACK: Small band gap {:.3} eV < {:.3} eV threshold",
                metrics.band_gap_ev, thresholds.gap_min_ev));
            return FallbackDecision {
                should_fallback: true,
                reason: FallbackReason::SmallBandGap { gap_ev: metrics.band_gap_ev },
                metrics: metrics.clone(),
        ...
        if metrics.cv_error > thresholds.cv_max {
            self.log(&format!("FALLBACK: CV error {:.2e} > threshold {:.2e}",
        ...
        if metrics.pole_stability_score < thresholds.pole_stability_min {
            self.log(&format!("FALLBACK: Pole stability {:.3} < threshold {:.3}",
        ...
        if metrics.causality_violation_score > thresholds.causality_max {
            self.log(&format!("FALLBACK: Causality violation {:.3} > threshold {:.3}",
        ...
        if metrics.sum_rule_error > thresholds.sum_rule_max {
            self.log(&format!("FALLBACK: Sum rule error {:.2e} > threshold {:.2e}",
...
mod tests {
    ...
    fn test_fallback_decision_small_gap() {
        ...
        assert!(matches!(decision.reason, FallbackReason::SmallBandGap { .. }));
    }
    ...
    fn test_fallback_decision_high_cv_error() {
        ...
        let decision = controller.evaluate(&metrics);
        assert!(decision.should_fallback);
        assert!(matches!(decision.reason, FallbackReason::CrossValidationError { .. }));
    }
