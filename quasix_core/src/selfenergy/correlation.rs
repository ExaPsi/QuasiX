...
pub fn integrate_sigma_c(
    ...
    if weights.len() != nfreq {
        return Err(QuasixError::DimensionMismatch(
            format!("Weights length {} != nfreq {}", weights.len(), nfreq)
        ));
    }

    if freqs.len() != nfreq {
        return Err(QuasixError::DimensionMismatch(
            format!("Frequencies length {} != nfreq {}", freqs.len(), nfreq)
        ));
    }
...
pub fn compute_sigma_c_intermediate(
    ...
    if nfreq_g != nfreq_w {
        return Err(QuasixError::DimensionMismatch(
            format!("Green function nfreq {} != W nfreq {}", nfreq_g, nfreq_w)
        ));
    }

    if naux_w1 != naux_w2 {
        return Err(QuasixError::DimensionMismatch(
            format!("W matrix not square: {} x {}", naux_w1, naux_w2)
        ));
    }

    if naux_l != naux_w1 {
        return Err(QuasixError::DimensionMismatch(
            format!("Lpq naux {} != W naux {}", naux_l, naux_w1)
        ));
    }
...
pub fn compute_sigma_c_diagonal(
    ...
    let sigma_c_iw = compute_sigma_c_intermediate(
        green_function,
        w_iw,
        lpq,
        nocc,
...
impl CorrelationSelfEnergy {
    ...
    pub fn compute_sigma_c_contour_deformation(
        ...
        Err(QuasixError::NotImplemented(
            "Legacy compute_sigma_c_contour_deformation - use compute_sigma_c_diagonal()".to_string()
        ))
    }
}
