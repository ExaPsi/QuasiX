...
impl JacobianOptimized {
    ...
    pub fn compute_jacobian_parallel(
        &self,
        x: &[f64],
        omega_max: f64,
    ) -> Vec<f64> {
    ...
    pub fn generate_grid_cached(
        ...
        // Generate new grid
        let (points, weights, jacobians) = self.generate_grid_internal(n, omega_max, transform_type)?;

        // Cache it
        self.cache.cache.insert(key, CachedGrid {
            points: points.clone(),
    ...
    fn generate_grid_internal(
        ...
        match transform_type {
            ...
            TransformType::Sinh => {
                self.sinh_transform_optimized(&nodes, &weights, omega_max)
            }
        }
    }
    ...
    pub fn validate_grid(
        ...
        let min_weight = weights.iter().filter(|&&w| w > 0.0).fold(f64::INFINITY, |a, &b| a.min(b));
    ...
    pub fn benchmark_jacobian_methods(&self, n: usize, omega_max: f64) {
        ...
        let _scalar_result: Vec<f64> = x.iter()
            .map(|&xi| self.safe_jacobian(xi, omega_max))
            .collect();
...
mod tests {
    ...
    fn test_safe_jacobian_no_overflow() {
        ...
        for &x in &x_values {
            ...
            assert!(jac <= computer.max_jacobian * omega_max,
    ...
    fn test_smooth_cutoff() {
        ...
        let x_values: Vec<f64> = (0..100)
            .map(|i| 1.0 - 1e-11 * (1.1_f64.powi(i)))
            .collect();

        let omega_max = 100.0;
        let jacobians: Vec<f64> = x_values.iter()
    ...
    fn test_simd_jacobian() {
        ...
        let scalar_result: Vec<f64> = x.iter()
    ...
    fn test_transform_types() {
        ...
        for transform in &[TransformType::Rational, TransformType::Exponential, TransformType::Sinh] {
            let (points, weights) = computer
            ...
            for i in 1..n {
                assert!(points[i] > points[i - 1],
                        "Points should be monotonic for {:?}", transform);
            }
        }
    }
}