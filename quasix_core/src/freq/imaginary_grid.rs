...
pub fn gauss_legendre_grid(nfreq: usize) -> Result<(Vec<f64>, Vec<f64>)> {
    ...
    if (sum_weights - 2.0).abs() > 1e-12 {
        return Err(QuasixError::NumericalError(
            format!("GL weight normalization error: sum = {}, expected 2.0", sum_weights)
        ));
    }
...
/// assert!((freqs[0] - 0.0).abs() < 1e-14);  // First point near 0
/// assert!((freqs[freqs.len()-1] - omega_max).abs() < 1e-12);  // Last point near ω_max
/// ```
...
/// assert!((freqs[0] - 0.0).abs() < 1e-10);  // First point near 0
/// assert!(freqs[freqs.len()-1] > 1000.0);    // Last point >> 1000 Ha
/// ```
...
pub fn create_imaginary_frequency_grid(
    ...
    if n_points < 2 {
        return Err(QuasixError::InvalidInput(
            format!("Number of points must be at least 2, got {n_points}")
        ));
    }
    if omega_max <= 0.0 {
        return Err(QuasixError::InvalidInput(
            format!("omega_max must be positive, got {omega_max}")
        ));
    }
    ...
    Ok((Array1::from_vec(frequencies), Array1::from_vec(transformed_weights)))
}
...
mod tests {
    ...
    fn test_gl_grid_symmetry() {
        ...
        for i in 0..nodes.len() {
            ...
            assert!(symmetric_diff < 1e-14,
                "Nodes not symmetric at index {}: diff = {}", i, symmetric_diff);
        }
    }
    ...
    fn test_gl_grid_range() {
        ...
        assert!(nodes.last().unwrap() >= &0.99 && nodes.last().unwrap() <= &1.0);  // Close to 1
    }
