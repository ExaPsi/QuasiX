...
impl AdaptiveFrequencyGrid {
        ...
        F: FnMut(f64) -> f64,
    {
        ...
        if self.config.verbose {
            eprintln!("Adaptive quadrature: coarse={:.6e}, fine={:.6e}, error={:.2e}",
        ...
        F: FnMut(f64) -> f64,
    {
        ...
        let tolerance = self.config.abs_tol.max(self.config.rel_tol * total_value.abs());
        ...
        while total_error > tolerance && n_evaluations < self.config.n_max * 4 {
            ...
            let left_result = self.integrate_interval(f, interval.left, mid, &gl4_nodes, &gl4_weights)?;
            n_evaluations += 4;

            // Compute integral on right half
            let right_result = self.integrate_interval(f, mid, interval.right, &gl4_nodes, &gl4_weights)?;
            n_evaluations += 4;
            ...
            if self.config.verbose && n_evaluations % 100 == 0 {
                eprintln!("  Refinement: n_eval={}, error={:.2e}", n_evaluations, total_error);
            }
        }
    ...
    pub fn generate_optimized_fixed_grid(&self, target_points: usize) -> Result<(Array1<f64>, Array1<f64>)> {
...
mod tests {
    ...
    fn test_adaptive_integration_polynomial() {
        ...
        eprintln!("Polynomial test: result={:.6}, error={:.2e}, n_eval={}",
                 result, error, n_eval);
    }
    ...
    fn test_adaptive_vs_fixed_comparison() {
        ...
        let (freqs_ref, weights_ref) = AdaptiveFrequencyGrid::generate_transformed_grid(100, 0.5).unwrap();
        ...
        eprintln!("Adaptive: {:.8}, Fixed(100): {:.8}, diff={:.2e}, n_eval={}",
