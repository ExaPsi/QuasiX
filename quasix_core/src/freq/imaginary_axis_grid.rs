...
impl ImaginaryAxisGrid {
    ...
    pub fn new(n_points: usize, omega_max: f64) -> Result<Self> {
        ...
        log::debug!(
            ...
            metadata.n_actual, metadata.xi_max, metadata.overflow_filtered
        );
    ...
    pub fn get_xi_values(&self) -> Array1<f64> {
        Array1::from_vec(
            self.frequencies.iter().map(|f| f.im).collect()
        )
    }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if relative_error > 0.1 {
            log::warn!(
                ...
                self.metadata.weight_sum, expected_sum, relative_error * 100.0
            );
        }
...
mod tests {
    ...
    fn test_error_estimate() {
        ...
        println!("Coarse points: {}, Fine points: {}",
