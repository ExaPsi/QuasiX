    ...
    unsafe fn apply_gaussian_broadening_avx2(
        ...
        for (omega_idx, &omega) in omega_grid.iter().enumerate() {
            ...
            for chunk_idx in 0..chunks {
                ...
                // Compute exponent
                let exponent = _mm256_mul_pd(
                    delta_e_sq,
                    _mm256_set1_pd(neg_inv_two_sigma_sq),
                );
        ...
        Zip::from(omega_grid)
            ...
            .for_each(|&omega, weight| {
                ...
                for (i, &energy) in self.qp_energies.iter().enumerate() {
                    ...
                    // Gaussian component
                    let gaussian =
                        gaussian_prefactor * (-delta_e * delta_e / two_sigma_sq).exp();

                    // Lorentzian component
                    let lorentzian =
                        lorentzian_prefactor / (delta_e * delta_e + gamma * gamma);
