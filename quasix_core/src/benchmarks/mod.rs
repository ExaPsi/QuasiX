...
impl MoleculeResult {
    ...
    pub fn passes_threshold(&self, max_dev: f64) -> bool {
        ...
        self.ea_deviation().is_none_or(|dev| dev.abs() <= max_dev * 1.5)
    }
}
...
impl StatisticalAnalyzer {
    ...
    pub fn analyze(&self, results: &[MoleculeResult]) -> BenchmarkStatistics {
        ...
        // Extract deviations
        let deviations: Vec<f64> = results.iter()
            .map(|r| r.ip_deviation())
            .collect();
    ...
    fn compute_regression(&self, results: &[MoleculeResult]) 
        -> (f64, f64, f64, f64) 
    {
...
impl BenchmarkRunner {
    ...
    pub fn run_benchmarks(
        ...
        let results = pool.install(|| {
            molecules.par_iter().map(|mol_data| {
                // Check cache first
                ...
                let ref_data = references.get(&mol_data.name)
                    .ok_or_else(|| QuasixError::ValidationError(
                        format!("No reference for {}", mol_data.name)
                    ))?;
                ...
                let result = MoleculeResult {
                    ...
                    orbital_deviations: self.compute_orbital_deviations(
                        &gw_result, ref_data
                    ),
                };
...
impl ReferenceDatabase {
    ...
    pub fn load_gw100() -> Result<Self> {
        ...
        // H2O reference
        data.insert("H2O".to_string(), ReferenceData {
        ...
        // NH3 reference
        data.insert("NH3".to_string(), ReferenceData {
        ...
        // CO reference
        data.insert("CO".to_string(), ReferenceData {
        ...
        // Benzene reference
        data.insert("benzene".to_string(), ReferenceData {
...
impl BenchmarkCache {
    ...
    fn load_cache(cache_dir: &str) -> Result<HashMap<String, MoleculeResult>> {
        ...
        let data = std::fs::read_to_string(&path)
            .map_err(|e| QuasixError::IoError(e.to_string()))?;
...
impl ValidationPipeline {
    ...
    pub fn validate(&self, results: &[MoleculeResult]) -> ValidationReport {
        ...
        let overall_pass = mad_pass && rmsd_pass && 
                          correlation_pass && pass_rate_pass;

        // Identify problem molecules
        let failed_molecules: Vec<String> = results.iter()
            .filter(|r| !r.passes_threshold(self.config.max_deviation_ev))
    ...
    fn generate_diagnostics(
        ...
        // Convergence issues
        let convergence_issues: Vec<_> = results.iter()
...
impl ValidationReport {
    ...
    pub fn format_report(&self) -> String {
        ...
        report.push_str(&format!(
            "  MAD ≤ 0.2 eV: {}\n",
            if self.mad_pass { "✓ PASS" } else { "✗ FAIL" }
        ));
        report.push_str(&format!(
            "  RMSD ≤ 0.24 eV: {}\n",
            if self.rmsd_pass { "✓ PASS" } else { "✗ FAIL" }
        ));
        report.push_str(&format!(
            "  R² ≥ 0.95: {}\n",
            if self.correlation_pass { "✓ PASS" } else { "✗ FAIL" }
        ));
        report.push_str(&format!(
            "  Pass rate ≥ 75%: {}\n\n",
            if self.pass_rate_pass { "✓ PASS" } else { "✗ FAIL" }
        ));
    ...
    pub fn to_csv(&self, results: &[MoleculeResult]) -> String {
        ...
        for result in results {
            csv.push_str(&format!(
                ...
                result.ip_ref,
                result.ip_deviation(),
                if result.passes_threshold(0.2) { "Yes" } else { "No" }
            ));
        }
