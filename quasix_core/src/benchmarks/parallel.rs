...
use super::{MoleculeResult, BenchmarkConfig};
...
impl ExecutionMetrics {
    ...
    pub fn print_summary(&self, n_threads: usize) {
        ...
        println!("Compute time:         {:.2}s", self.compute_time.as_secs_f64());
        println!("Queue time:           {:.2}s", self.queue_time.as_secs_f64());
        println!("Parallel efficiency:  {:.1}%", self.efficiency(n_threads) * 100.0);
        println!("Tasks completed:      {}", self.tasks_completed);
        println!("Tasks failed:         {}", self.tasks_failed);

        if self.total_steal_attempts > 0 {
            println!("Work steal rate:      {:.1}% ({}/{})",
        ...
        if !self.thread_utilization.is_empty() {
            let avg_util: f64 = self.thread_utilization.iter().sum::<f64>()
                / self.thread_utilization.len() as f64;
...
impl MemoryAwareScheduler {
    ...
    pub fn release(&self, memory_estimate: usize) {
        self.current_usage.fetch_sub(memory_estimate, Ordering::SeqCst);
    }
...
impl ParallelBenchmarkExecutor {
    ...
    pub fn execute_benchmarks(
            ...
            "memory" => {
                ...
                self.batch_scheduler.create_memory_batches(
                    ...
                    |mol: &MolecularData| {
                        ...
                        (mol.nbasis * mol.nbasis * mol.nbasis * 3 * 8) as f64 / (1024.0 * 1024.0 * 1024.0)
                    },
                )
            },
        ...
        // Execute in parallel
        let results = self.scheduler.execute(move |mol_data: &MolecularData| -> Result<MoleculeResult> {
...
mod tests {
    ...
    fn test_work_stealing_scheduler() {
        ...
        let items: Vec<WorkItem<String>> = (0..10).map(|i| {
            WorkItem {
                id: i,
    ...
    fn test_dynamic_batch_scheduler() {
        ...
        let items: Vec<WorkItem<String>> = vec![
            WorkItem { id: 0, priority: 1, estimated_cost: 10.0, data: "large".into() },
            WorkItem { id: 1, priority: 2, estimated_cost: 5.0, data: "medium".into() },
            WorkItem { id: 2, priority: 3, estimated_cost: 3.0, data: "small1".into() },
            WorkItem { id: 3, priority: 4, estimated_cost: 2.0, data: "small2".into() },
            WorkItem { id: 4, priority: 5, estimated_cost: 1.0, data: "tiny".into() },
        ];
