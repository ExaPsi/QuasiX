...
impl DFTensorsS24 {
    ...
    fn validate_schema(&self) -> Result<()> {
        ...
        if max_asymmetry > 1e-10 {
            anyhow::bail!(
                "metric_2c is not symmetric: max asymmetry = {}",
                max_asymmetry
            );
        }
...
mod hdf5_impl {
    ...
    pub(super) fn validate_schema_structure(file: &File, meta: &DFMetadata) -> Result<()> {
        ...
        file.group("metadata")
            .context("Missing /metadata group")?;
    ...
    fn verify_dataset_shape(group: &Group, name: &str, expected_shape: &[usize]) -> Result<()> {
        ...
        if shape != expected_shape {
            anyhow::bail!(
                "Dataset {} shape mismatch: expected {:?}, got {:?}",
                name,
                expected_shape,
                shape
            );
...
mod tests {
    ...
    fn test_write_invalid_schema_version_fails() {
        ...
        assert!(
            err_msg.contains("Unsupported schema version") || err_msg.contains("Schema validation failed"),
            "Expected error about schema version, got: {}",
            err_msg
        );
    }
