...
pub fn estimate_condition_power_iteration(
    ...
    let max_eigenvalue = power_iteration_largest(matrix, max_iterations, tolerance)
        .unwrap_or(1.0);

    // Estimate smallest eigenvalue via inverse iteration
    let min_eigenvalue = inverse_iteration_smallest(matrix, max_iterations, tolerance)
        .unwrap_or(1e-15);
...
fn inverse_iteration_smallest(
    ...
    let matrix_inv = if let Ok(inv) = matrix.inv() { inv } else {
...
pub fn apply_tikhonov_regularization(
...
pub fn analyze_condition(
    ...
    let max_eig = power_iteration_largest(matrix, config.max_iterations, config.tolerance)
        .unwrap_or(1.0);
...
#[must_use] pub fn is_well_conditioned(
    matrix: &Array2<Complex64>,
    threshold: f64,
) -> bool {
