...
impl ConvergenceMonitor {
    ...
    fn update_statistics(&self) {
        ...
        let var_z = z_changes
            .iter()
            .map(|x| (x - mean_z).powi(2))
            .sum::<f64>()
            / z_changes.len() as f64;
...
mod tests {
    ...
    fn test_oscillation_score() {
        ...
        assert!(score > 0.5, "Expected high oscillation score for alternating pattern");
        ...
        assert!(score < 0.3, "Expected low oscillation score for monotonic decrease");
    }
    ...
    fn test_stagnation_score() {
        ...
        assert!(score < 0.1, "Short converged sequence should not trigger stagnation");
        ...
        assert!(score > 0.5, "Constant values above threshold should trigger stagnation: {}", score);
