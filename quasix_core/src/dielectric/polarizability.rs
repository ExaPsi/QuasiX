...
pub fn compute_polarizability_p0(
    ...
    mo_occ: &Array1<f64>,
    freqs: &Array1<f64>,
) -> Result<Array3<f64>> {  // CHANGED: Returns REAL Array3<f64> for CD method!
    let (n_occ, n_virt, n_aux) = ia_P.dim();
    let nfreq = freqs.len();

    // Identify occupied/virtual orbitals
    let occ_indices: Vec<usize> = mo_occ.iter()
        .enumerate()
        .filter(|(_, &occ)| occ > 0.5)
        .map(|(i, _)| i)
        .collect();
    let vir_indices: Vec<usize> = mo_occ.iter()
        .enumerate()
        .filter(|(_, &occ)| occ < 0.5)
        .map(|(i, _)| i)
        .collect();
    ...
    if occ_indices.len() != n_occ || vir_indices.len() != n_virt {
        return Err(QuasixError::InvalidInput(
            format!("Occupation mismatch: expected {}/{}, got {}/{}",
                    n_occ, n_virt, occ_indices.len(), vir_indices.len())
        ));
    }
...
pub fn compute_p0_single_frequency(
    ...
    _mo_occ: &Array1<f64>,
    _omega: f64,
) -> Result<ndarray::Array2<f64>> {  // CHANGED: f64 for CD method
...
mod tests {
    ...
    fn test_p0_symmetry() {
        ...
        for f in 0..nfreq {
            for p in 0..n_aux {
                for q in 0..n_aux {
                    ...
                    assert!(diff < 1e-14, "P0[{},{},{}] not symmetric: diff = {}", f, p, q, diff);
                }
            }
        }
        ...
        for f in 0..nfreq {
            for p in 0..n_aux {
                for q in 0..n_aux {
                    assert!(p0[[f, p, q]].is_finite(),
                            "P0[{},{},{}] is not finite: {}", f, p, q, p0[[f, p, q]]);
                }
            }
        }
    }
    ...
    fn test_p0_prefactor_sign() {
        ...
        for p in 0..n_aux {
            for q in 0..n_aux {
                assert!(p0[[0, p, q]] > 0.0,
                       "P0[{},{}] should be positive: {}", p, q, p0[[0, p, q]]);
                // CD method: P₀ is exactly real (no imaginary part)
                assert!(p0[[0, p, q]].is_finite(),
                       "P0[{},{}] not finite: {}", p, q, p0[[0, p, q]]);
            }
        }
    }
}
