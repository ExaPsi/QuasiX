...
pub fn compute_screened_interaction(
    ...
    v_aux: &Array2<f64>,
    _freqs: &Array1<f64>,
) -> Result<Array3<f64>> {   // CHANGED: Returns REAL Array3<f64>
    // Validate dimensions
    ...
    if n_aux != n_aux_2 {
        return Err(QuasixError::DimensionMismatch(
            format!("v_aux must be square: expected {}x{}, got {}x{}",
    ...
    if n_aux_p0 != n_aux_p0_2 || n_aux_p0 != n_aux {
        return Err(QuasixError::DimensionMismatch(
            format!("P0 dimension mismatch: expected [freq, {}, {}], got [freq, {}, {}]",
...
pub fn compute_screened_interaction_with_vsqrt(
...
) -> Result<Array3<f64>> {   // CHANGED: f64 for CD method
    ...
    let v_sqrt = match v_sqrt_opt {
        Some(v_sqrt_provided) => {
            ...
            if n_aux_v != n_aux_v2 {
                return Err(QuasixError::DimensionMismatch(
                    format!("v_sqrt must be square: got {}x{}", n_aux_v, n_aux_v2)
                ));
            }

            if n_aux_v != n_aux_p0 {
                return Err(QuasixError::DimensionMismatch(
                    format!("v_sqrt dimension mismatch: expected {}, got {}", n_aux_p0, n_aux_v)
                ));
            }
...
pub fn compute_W_single_frequency(
    ...
    v_sqrt: &Array2<f64>,
    n_aux: usize,
) -> Result<Array2<f64>> {   // CHANGED: f64 for CD method
    ...
    let epsilon_inv = match epsilon.invc() {
        ...
        Err(chol_err) => {
            ...
            eprintln!("[W] Cholesky inversion failed, falling back to LU: {:?}", chol_err);
...
pub fn compute_W_single_frequency_diagnostic(
    ...
    if print_diagnostics {
        ...
        eprintln!("[RUST W DIAG]   epsilon[0,0] = {:.10e} (real)", epsilon[[0, 0]]);
        eprintln!("[RUST W DIAG]   epsilon[0,1] = {:.10e} (real)", epsilon[[0, 1]]);
    ...
    if print_diagnostics {
        ...
        eprintln!("[RUST W DIAG]   epsilon_sym norm = {:.10e}", epsilon_sym_norm);
    }

    // Step 4: Invert ε to get ε^{-1} (real matrix inversion)
    let epsilon_inv = epsilon.inv()
        .map_err(|e| QuasixError::NumericalError(
            format!("Failed to invert dielectric function: {:?}", e)
        ))?;

    if print_diagnostics {
        let epsilon_inv_norm = (epsilon_inv.iter().map(|x| x * x).sum::<f64>()).sqrt();
        eprintln!("[RUST W DIAG] Step 4: W_tilde = epsilon^{{-1}}");
        eprintln!("[RUST W DIAG]   W_tilde[0,0] = {:.10e} (real)", epsilon_inv[[0, 0]]);
        eprintln!("[RUST W DIAG]   W_tilde[0,1] = {:.10e} (real)", epsilon_inv[[0, 1]]);
    ...
    if print_diagnostics {
        ...
        eprintln!("[RUST W DIAG]   W norm (before sym) = {:.10e}", W_xi_norm_before_sym);
    }
...
pub fn compute_metric_sqrt(
    ...
    let (eigvals, eigvecs) = v_aux.to_owned().eigh(UPLO::Lower)
        .map_err(|e| QuasixError::NumericalError(
            format!("Eigendecomposition failed: {:?}", e)
        ))?;
    ...
    let max_err = v_aux.iter().zip(reconstructed.iter())
        .map(|(a, b)| (a - b).abs())
        .fold(0.0, f64::max);

    if max_err > 1e-10 {
        return Err(QuasixError::NumericalError(
            format!("Eigendecomposition sqrt reconstruction error: {:.3e}", max_err)
        ));
    }
...
pub fn debug_compute_screening_intermediates(
    ...
    let max_error = v_aux.iter().zip(reconstructed.iter())
        .map(|(a, b)| (a - b).abs())
        .fold(0.0, f64::max);
...
mod tests {
    ...
    fn test_w_symmetry() {
        ...
        // Compute W
        let W = compute_screened_interaction(&P0, &v_aux, &freqs)
            .expect("W computation failed");
        ...
        for ifreq in 0..nfreq {
            for i in 0..n_aux {
                for j in 0..n_aux {
                    ...
                    assert!(diff < 1e-12, "W not symmetric at [{},{},{}]: diff = {}", ifreq, i, j, diff);
                }
            }
        }
    }
    ...
    fn test_w_finite_values() {
        ...
        for val in W.iter() {
            assert!(val.is_finite(),
                   "W contains non-finite value: {}", val);
        }
    }
    ...
    fn test_cholesky_correctness() {
        ...
        let v_sqrt = compute_metric_sqrt(&v)
