...
use super::gw100::{
    BenchmarkResult, SimdStatistics,
...
impl PyValidationStats {
    ...
    fn to_dict(&self) -> HashMap<String, Py<PyAny>> {
        Python::attach(|py| {
            ...
            dict.insert("mad".to_string(), self.mad.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("rmse".to_string(), self.rmse.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("max_error".to_string(), self.max_error.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("correlation".to_string(), self.correlation.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("n_samples".to_string(), self.n_samples.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("mad_ci_lower".to_string(), self.mad_ci_lower.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("mad_ci_upper".to_string(), self.mad_ci_upper.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("n_outliers".to_string(), self.n_outliers.into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("outlier_indices".to_string(), self.outlier_indices.clone().into_pyobject(py).unwrap().unbind().into_any());
            dict.insert("per_molecule_mad".to_string(), self.per_molecule_mad.clone().into_pyobject(py).unwrap().unbind().into_any());
            dict
        })
    }
...
pub fn register_module(py: Python<'_>, parent_module: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    validation_module.add_function(wrap_pyfunction!(py_compute_correlation, &validation_module)?)?;
    validation_module.add_function(wrap_pyfunction!(py_detect_outliers, &validation_module)?)?;
    validation_module.add_function(wrap_pyfunction!(py_compute_validation_stats, &validation_module)?)?;
