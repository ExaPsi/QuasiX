...
use std::arch::x86_64::{
    _mm256_add_pd, _mm256_and_pd, _mm256_loadu_pd, _mm256_set1_pd,
...
pub fn compute_validation_stats(results: &[BenchmarkResult]) -> Result<ValidationStats> {
    ...
    let all_deviations: Vec<f64> = results.iter().flat_map(|r| &r.deviations).copied().collect();
...
fn bootstrap_ci_simple(calculated: &[f64], reference: &[f64], n_bootstrap: usize) -> Result<(f64, f64)> {
    use rand::prelude::*;
...
impl OptimizedArray {
    ...
    pub fn get_data(&self) -> Option<Vec<f64>> {
        if let Some(ref data) = self.data {
            Some(data.clone())
        } else { self.compressed.as_ref().map(|compressed| Self::decompress(compressed)) }
    }
...
pub fn run_evgw_calculation(
    ...
    let qp_energies_ev: Vec<f64> = result.qp_energies.iter()
        .map(|&e| e * HA_TO_EV)
        .collect();
    ...
    move |_task: &BenchmarkTask| {
        ...
        Err(crate::common::QuasixError::NotImplemented(
            "Full evGW executor requires PySCF interface for molecular data loading. \
             Use Python bindings or provide precomputed HDF5 data.".to_string()
        ))
    }
}
...
mod tests {
    ...
    #[test]
    #[ignore]  // This test is computationally intensive, run with --ignored flag
    fn test_evgw_full_calculation() {
        ...
        match result {
            Ok(benchmark_result) => {
                ...
                assert!(benchmark_result.z_factors.iter().all(|&z| z > 0.0 && z <= 1.0));
            }
            Err(e) => {
