...
#[cfg(test)]
mod tests {
    use crate::df::mo_transform::{generate_mock_mo_coefficients, transform_mo_3center, TransformConfig};
    use crate::df::simd_ops::{check_simd_support, simd_matmul_transpose};
    use ndarray::{Array2, Array3};

    /// Generate small test data
    fn generate_test_data(n_ao: usize, n_aux: usize, n_occ: usize, n_vir: usize)
    ...
    fn test_simd_matmul_vs_ndarray() {
        ...
        for size in [8, 16, 32, 64].iter() {
            ...
            if simd_support.has_high_perf_simd() {
                println!("✓ Size {} passed with AVX2/FMA (max error: {:.2e})", size, max_error);
            } else {
                println!("✓ Size {} passed with scalar fallback (max error: {:.2e})", size, max_error);
            }
        }
    }
    ...
    fn test_mo_transform_simd_correctness() {
        ...
        for (name, n_ao, n_aux, n_occ, n_vir) in test_cases {
            ...
            // Compare results
            let max_error = result_simd.iter()
            ...
            println!("✓ {} MO transform validated (max error: {:.2e})", name, max_error);
        }
    }
    ...
    fn test_lock_free_parallel_correctness() {
        ...
        use crate::df::parallel::{transform_mo_3center_parallel_optimized, ThreadPoolConfig, BlockingStrategy};
        ...
        println!("✓ Lock-free parallel transformation validated (max error: {:.2e})", max_error);
    }

    #[test]
    fn test_thread_coordination_no_deadlock() {
        // Ensure thread coordination doesn't cause deadlocks
        use crate::df::parallel::{ThreadPoolConfig, transform_mo_3center_parallel_optimized, BlockingStrategy};

        let (j3c_ao, c_occ, c_vir) = generate_test_data(12, 132, 8, 4);  // H2CO size

        // Test different configurations sequentially
        for blas_threads in [1, 2].iter() {
            ...
            assert!(result.is_ok(), "Thread config with blas_threads={} failed", blas_threads);
            println!("✓ Thread coordination with blas_threads={} succeeded", blas_threads);
        }
    }
    ...
    fn test_simd_alignment() {
        ...
        for size in [7, 13, 17, 23].iter() {  // Non-multiples of 4 (AVX2 width)
            let m = *size;
            let k = *size;
            let n = *size;
